...
// 2. USER_CONFIG/dgmo/themes/*.json
// 3. PROJECT_ROOT/.dgmo/themes/*.json
// 4. CWD/.dgmo/themes/*.json
func LoadThemesFromDirectories(userConfig, projectRoot, cwd string) error {
	...
	dirs := []string{
		...
		filepath.Join(projectRoot, ".dgmo", "themes"),
	}
	if cwd != projectRoot {
		dirs = append(dirs, filepath.Join(cwd, ".dgmo", "themes"))
	}
