...
func TestLoadThemesFromJSON(t *testing.T) {
	...
	expectedThemes := []string{"tokyonight", "dgmo", "everforest", "ayu"}
...
func TestLoadThemesFromDirectories(t *testing.T) {
	...
	os.MkdirAll(filepath.Join(userConfig, "dgmo", "themes"), 0755)
	os.MkdirAll(filepath.Join(projectRoot, ".dgmo", "themes"), 0755)
	os.MkdirAll(filepath.Join(cwd, ".dgmo", "themes"), 0755)
	...
	os.WriteFile(filepath.Join(userConfig, "dgmo", "themes", "override-test.json"), []byte(testTheme1), 0644)
	os.WriteFile(filepath.Join(projectRoot, ".dgmo", "themes", "override-test.json"), []byte(testTheme2), 0644)
	os.WriteFile(filepath.Join(cwd, ".dgmo", "themes", "override-test.json"), []byte(testTheme3), 0644)
