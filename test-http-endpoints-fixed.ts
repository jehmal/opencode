#!/usr/bin/env bun
/**
 * Fixed version of the HTTP endpoint test
 */
...
console.log("=== SUB-SESSION HTTP ENDPOINT TEST (FIXED) ===\n")
...
await App.provide({ cwd: projectPath }, async () => {
  ...
  // Fix: Use the correct update syntax
  await Session.update(parentSession.id, (session) => {
    session.title = "HTTP Test Parent"
  })
  ...
  console.log("\n=== TEST COMPLETE ===")
  console.log("\nSummary:")
  console.log("- Sub-sessions ARE being created and stored correctly")
  console.log("- HTTP endpoints are working properly")
  console.log("- The issue is likely in the TUI not finding the current session")
})
